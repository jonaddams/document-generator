...
export default function DownloadStep() {
  ...
  const initializePdfViewer = useCallback(async () => {
    ...
    try {
      ...
      dispatch({ type: 'SET_ERROR', payload: `PDF document validation failed: ${errorMessage}` });
      return;
    }
    ...
    try {
      ...
      if (window.PSPDFKit) {
        try {
          ...
          try {
            ...
            console.log('ℹ️ No existing PSPDFKit instance to unload (this is normal)');
          }
        }
      ...
      if (window.NutrientViewer) {
        ...
        while (retryCount < maxRetries) {
          try {
            ...
            retryCount++;
            console.warn(`⚠️ PDF viewer load attempt ${retryCount} failed:`, loadError);
      ...
      dispatch({ type: 'SET_ERROR', payload: error instanceof Error ? error.message : 'PDF viewer initialization failed' });
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="h-full flex flex-col space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        ...
        <div className="text-center p-4 bg-white border border-gray-200 rounded-lg">
          <div className="text-2xl font-bold text-indigo-600">
            {state.pdfDocument ? `${Math.round(state.pdfDocument.byteLength / 1024)} KB` : 'N/A'}
          </div>
